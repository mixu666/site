 ...
 *  @version    1.0
 */
 class MapController extends Oibs_Controller_CustomController
{
    ...
    {
        // get latitude & longitude from params
        $lat = $this->_request->getParam('lat');
        $lng = $this->_request->getParam('lng');

        if ($lat && $lng) {
            // use them to search the db
            $coordModel = new Default_Model_Coordinates();
            $this->view->results = $coordModel->getAllItemsByCoordinates($lat, $lng);
        }
    }
}
